...
int main(int argc, char** argv) {
    ...
    //cout<<countSimilarSubstring("o medicamento zofran tem muita utilidade para pacientes zofram sofram","zofran")<<endl;;
                int n=0;
                //inicio=de1->text().toUTF8();
                //fim=de2->text().toUTF8();
                FILE *fp;
                fp=fopen("sinonimos.xls","w");
                if(fp==NULL){
                        printf("Arquivo nao disponivel...");
                        exit(1);
                }
                ...
                fprintf(fp,"<Row>\n\
                <Cell><Data ss:Type=\"String\">Medicamento</Data></Cell>\n\
                <Cell><Data ss:Type=\"String\">Sinonimo</Data></Cell>\n\
                fprintf(fp,"</Row>\n");

                for(int i=0;i<reg_sinonimo.size();i++)
                for(int j=0;i<reg_sinonimo[i].size();i++){
                fprintf(fp,"<Row>\n\
                <Cell><Data ss:Type=\"String\">%s</Data></Cell>\n\
                <Cell><Data ss:Type=\"String\">%s</Data></Cell>\n\
</Row>\n",reg_sinonimo[i].nome,reg_sinonimo[i].nome_alternativo[j]);
                        }
                }
                fprintf(fp,"</Table>\n");
                fprintf(fp,"</Worksheet>\n");
